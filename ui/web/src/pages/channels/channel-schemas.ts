...
const chatBehaviorOverrideFields: FieldDef[] = [
  { key: "chat_behavior.enabled", label: "Human-like Delivery", type: "select", options: inheritToggleOptions, defaultValue: "inherit", help: "Channel-level override for delivery behavior. Channel settings win over Agent and Workspace settings." },
  { key: "chat_behavior.intermediate_replies.enabled", label: "Intermediate Replies", type: "select", options: inheritToggleOptions, defaultValue: "inherit", help: "Send natural progress bubbles during long or tool-heavy runs. These messages are not added to session history." },
  { key: "chat_behavior.intermediate_replies.mode", label: "Intermediate Reply Mode", type: "select", options: [{ value: "inherit", label: "Inherit from gateway" }, { value: "sidecar_generated", label: "Sidecar-generated" }, { value: "off", label: "Off" }], defaultValue: "inherit", help: "Choose whether progress updates are generated by the delivery model or disabled for this channel." },
  { key: "chat_behavior.intermediate_replies.provider", label: "Intermediate Provider", type: "text", placeholder: "Use agent provider", help: "Optional provider for progress bubbles. Leave empty to inherit the agent provider; pick a fast, cheap provider here." },
  { key: "chat_behavior.intermediate_replies.model", label: "Intermediate Model", type: "text", placeholder: "Use agent model", help: "Optional model for progress bubbles. Leave empty to inherit the agent model; custom model names are allowed." },
  { key: "chat_behavior.intermediate_replies.timeout_ms", label: "Intermediate Timeout (ms)", type: "number", defaultValue: 2500, help: "Maximum time to wait for the delivery model before skipping an intermediate update." },
  { key: "chat_behavior.intermediate_replies.max_tokens", label: "Intermediate Max Tokens", type: "number", defaultValue: 60, help: "Token cap for each generated progress bubble. Keep this small so updates stay short." },
  { key: "chat_behavior.intermediate_replies.max_chars", label: "Intermediate Max Chars", type: "number", defaultValue: 180, help: "Character cap after generation so progress bubbles do not become full answers." },
  { key: "chat_behavior.quick_ack.enabled", label: "Quick Acknowledgement", type: "select", options: inheritToggleOptions, defaultValue: "inherit", help: "Send a short natural acknowledgement before long work starts. This message is not added to session history." },
  { key: "chat_behavior.quick_ack.mode", label: "Quick Ack Mode", type: "select", options: quickAckModeOptions, defaultValue: "inherit", help: "Generated modes use the configured delivery provider/model. Fixed template mode is the only mode that sends template text." },
  { key: "chat_behavior.quick_ack.provider", label: "Quick Ack Provider", type: "text", placeholder: "Use agent provider", help: "Optional provider for quick acknowledgements. Leave empty to inherit the agent provider; pick a fast, cheap provider here." },
  { key: "chat_behavior.quick_ack.model", label: "Quick Ack Model", type: "text", placeholder: "Use agent model", help: "Optional model for quick acknowledgements. Leave empty to inherit the agent model; custom model names are allowed." },
  { key: "chat_behavior.quick_ack.timeout_ms", label: "Quick Ack Timeout (ms)", type: "number", defaultValue: 2500, help: "Maximum time to wait for the acknowledgement model before skipping generated acknowledgement." },
  { key: "chat_behavior.quick_ack.max_tokens", label: "Quick Ack Max Tokens", type: "number", defaultValue: 40, help: "Token cap for the acknowledgement. Keep this small so it reads like a receipt, not an answer." },
  { key: "chat_behavior.quick_ack.max_chars", label: "Quick Ack Max Chars", type: "number", defaultValue: 120, help: "Character cap after generation so acknowledgement bubbles stay compact." },
  { key: "chat_behavior.final_split.enabled", label: "Final Reply Splitting", type: "select", options: inheritToggleOptions, defaultValue: "inherit", help: "Split long final answers into multiple channel messages. This is separate from reasoning and progress bubbles." },
];
