...
# Analyst-facing shortlist entries carry a trimmed summary. Upstream shipped
# title+URL only, which forced every analyst to infer the story from a headline —
# and made framing sources (podcast episodes, whose value IS the description)
# worthless. 400 chars keeps candidates.json well inside read_file's 50k cap.
_SUMMARY_REF_CHARS = 400


def _item_ref(item) -> dict[str, Any]:
    summary = item.summary or ""
    if len(summary) > _SUMMARY_REF_CHARS:
        summary = summary[:_SUMMARY_REF_CHARS].rstrip() + "…"
    return {
