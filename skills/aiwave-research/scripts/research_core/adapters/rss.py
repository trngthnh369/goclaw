...
def _child_text(node: ElementTree.Element, names: tuple[str, ...]) -> str:
    ...
    for child in node:
        ...
        if local in names and child.text:
            return normalize_text(child.text)
    # Media RSS nests the real text one level down (YouTube puts the whole video
    # description in media:group/media:description). A direct-children-only scan
    # returns an empty summary for those feeds, which strips the item of every
    # word the scorer relies on.
    for child in node:
        if child.tag.rsplit("}", 1)[-1].lower() != "group":
            continue
        for grandchild in child:
            local = grandchild.tag.rsplit("}", 1)[-1].lower()
            if local in names and grandchild.text:
                return normalize_text(grandchild.text)
    return ""
