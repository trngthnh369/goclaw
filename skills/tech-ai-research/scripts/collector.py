...
from research_core.adapters import GitHubReleasesAdapter, HackerNewsAdapter, RSSAdapter, SourceHealth
...
def build_adapter(source: dict, http: HttpClient):
    ...
    if source_type == "hacker_news":
        return HackerNewsAdapter(source, http)
    if source_type == "github_releases":
        return GitHubReleasesAdapter(source, http)
    if source_type == "rss":
